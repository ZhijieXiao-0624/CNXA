...
parser.add_argument('--weights', default="D://PyCharm/xiaozhijie/modify_project/CNXA/CNX_X/weights/convnext_base_1k_224_ema.pth",
...
def train_one_epoch(epoch, model, criterion_class, optimizer, train_loader, use_gpu):
    # 打印信息
    losses = AverageMeter()
...
def main():
    ...
    nw = min([os.cpu_count(), args.freeze_train_batch if args.freeze_train_batch > 1 else 0,
    ...
    # 冻结训练，即载入的预训练模型部分不参与训练，此时占用显存较小
    if args.freeze_layers:
        if args.start_epoch < args.freeze_epoch:  # 判断读档训练是否小于冻结训练
            print("Prepare for {} freeze trunk training!".format(args.freeze_epoch - args.start_epoch))
            for name, para in model.named_parameters():
                # 只训练head、CA和last_norm层
                if "head" not in name and "CA" not in name and "last_norm" not in name:
    ...
    if start_epoch < args.freeze_epoch:
        ...
        for epoch in range(start_epoch, args.freeze_epoch):
            ...
            train_one_epoch(epoch, model, criterion_class, optimizer, train_loader_freeze, use_gpu)
            ...
            print(
                "==> Best top1 {:.1%}\tBest top5 {:.1%}\tachieved at epoch {}".format(best_top1, best_top5, best_epoch))
    else:
    ...
    args.freeze_layers = False  # 再冻结训练完成后进行解冻训练，即训练所有参数
    ...
    if args.freeze_epoch < args.max_epoch:
        ...
        for epoch in range(args.freeze_epoch, args.max_epoch):
            ...
            train_one_epoch(epoch, model, criterion_class, optimizer, train_loader_unfreeze, use_gpu)
            # val(model, valloader, use_gpu)
